...
const LeafDeco = () => (
  <svg width="120" height="120" viewBox="0 0 120 120" fill="none" opacity="0.06">
    <path d="M10 110 C10 110 20 20 110 10 C110 10 60 40 50 110 Z" fill="#f97316"/>
...
const BADGES = ["🌿 100% Organic", "🚚 Free Delivery $99+", "♻️ Eco Packaging", "⭐ 4.9 Rated"];
...
const Footer = () => {
  ...
  return (
    <>
      ...
      <footer className="gr-footer" style={{ background:"#0f0f0f", position:"relative", overflow:"hidden" }}>

        {/* ── Decorative leaves ── */}
        <div style={{ position:"absolute", top:0, right:0, pointerEvents:"none" }}><LeafDeco /></div>
        <div style={{ position:"absolute", bottom:60, left:-20, pointerEvents:"none", transform:"rotate(180deg)" }}><LeafDeco /></div>

        {/* ── Trust Badges Strip ── */}
        <div className="gr-badges-row" style={{ borderBottom:"1px solid rgba(255,255,255,0.06)", padding:"16px 24px", background:"#e4e4e7" }}>
          <div style={{ maxWidth:1280, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"center", gap:12, flexWrap:"wrap" }}>
            {BADGES.map(b => (
              <div key={b} className="gr-badge">{b}</div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"40px 24px 32px" }}>
          ...
          <div className="gr-footer-top" style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:48, marginBottom:56 }}>

            {/* Brand */}
            <div style={{ maxWidth:320 }}>
              <a href="/" style={{ textDecoration:"none", display:"inline-block", marginBottom:20 }}>
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontWeight:700, fontSize:34, color:"#fff", letterSpacing:"-0.03em", lineHeight:1 }}>
                  Gr<span style={{ color:"#f97316" }}>O</span>cify
                  <sup style={{ fontSize:"10px", fontFamily:"'DM Sans',sans-serif", fontWeight:600, letterSpacing:"0.12em", color:"#52525b", marginLeft:"3px" }}>FRESH</sup>
                </div>
              </a>
              <p style={{ fontSize:13, color:"#71717a", lineHeight:1.8, margin:"0 0 28px" }}>
                Farm-to-table freshness delivered to your door. We source directly from organic farms to bring you the finest produce every day.
              </p>
              <div style={{ display:"flex", gap:10 }}>
                {SOCIALS.map(s => (
                  <a key={s.label} href={s.href} className="gr-social-btn" aria-label={s.label}>{s.icon}</a>
                ))}
              </div>
            </div>
            ...
            <div style={{ maxWidth:400, flex:1 }}>
              <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(249,115,22,0.10)", border:"1px solid rgba(249,115,22,0.20)", borderRadius:999, padding:"4px 14px", marginBottom:16 }}>
                <span style={{ fontSize:10, fontWeight:700, letterSpacing:"0.12em", textTransform:"uppercase", color:"#f97316" }}>Newsletter</span>
              </div>
              <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:28, fontWeight:700, color:"#fff", margin:"0 0 8px", lineHeight:1.2 }}>
                Get Fresh Deals<br/><span style={{ color:"#f97316" }}>Every Week</span>
              </h3>
              <p style={{ fontSize:13, color:"#71717a", margin:"0 0 20px", lineHeight:1.7 }}>
                Subscribe for exclusive offers, seasonal picks, and farm-fresh updates.
              </p>
              {subscribed ? (
                <div style={{ display:"flex", alignItems:"center", gap:10, background:"rgba(22,163,74,0.10)", border:"1px solid rgba(22,163,74,0.25)", borderRadius:12, padding:"14px 18px" }}>
                  ...
                  <span style={{ fontSize:13, color:"#16a34a", fontWeight:600 }}>You're subscribed! Welcome to the Grocify family 🌿</span>
                </div>
              ) : (
                <div className="gr-newsletter-wrap" style={{ display:"flex", gap:10 }}>
                  <input className="gr-newsletter-input" type="email" placeholder="your@email.com" value={email} onChange={e => setEmail(e.target.value)}
                    onKeyDown={e => e.key === "Enter" && handleSubscribe()}/>
                  <button className="gr-sub-btn" onClick={handleSubscribe}>Subscribe</button>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Contact Info */}
          <div style={{ display:"flex", gap:32, flexWrap:"wrap", marginBottom:48, padding:"28px 32px", background:"rgba(255,255,255,0.025)", borderRadius:20, border:"1px solid rgba(255,255,255,0.06)" }}>
            {[
              { icon:"📍", label:"Address",  value:"42 Green Market Street, Farm District, NY 10001" },
              { icon:"📞", label:"Phone",    value:"+1 (800) GROCIFY"                                 },
              { icon:"✉️", label:"Email",    value:"hello@grocify.fresh"                              },
              { icon:"🕐", label:"Hours",    value:"Mon–Sat: 7am – 9pm"                              },
            ].map(item => (
              <div key={item.label} style={{ display:"flex", alignItems:"flex-start", gap:12, flex:"1 1 200px" }}>
                <span style={{ fontSize:18, lineHeight:1 }}>{item.icon}</span>
                <div>
                  <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.10em", textTransform:"uppercase", color:"#52525b", marginBottom:4 }}>{item.label}</div>
                  <div style={{ fontSize:13, color:"#a3a3a3", lineHeight:1.5 }}>{item.value}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Bottom Bar */}
          <div className="gr-bottom-row" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, flexWrap:"wrap" }}>
            <p style={{ fontSize:12, color:"#3f3f46", margin:0 }}>
              © 2026 <span style={{ color:"#f97316" }}>Grocify Fresh</span>. All rights reserved. Made with 🌿 for fresh food lovers.
            </p>
            <div style={{ display:"flex", gap:20, alignItems:"center" }}>
              {["Terms of Service","Privacy Policy","Cookie Policy"].map(link => (
                <a key={link} href="#" className="gr-bottom-link">{link}</a>
              ))}
            </div>
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              {["VISA","MC","AMEX","PayPal"].map(p => (
                <div key={p} style={{ background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:6, padding:"4px 10px", fontSize:9, fontWeight:800, letterSpacing:"0.08em", color:"#71717a" }}>{p}</div>
              ))}
            </div>
          </div>
        </div>
