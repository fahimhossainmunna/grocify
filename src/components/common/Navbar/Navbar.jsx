...
const HeartIcon = ({ filled }) => (<svg width="22" height="22" viewBox="0 0 24 24" fill={filled ? "#f97316" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>);
...
const Navbar = () => {
  ...
  // ✅ Redux selectors — useCart() এর বদলে
    ...
    <>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height:"68px" }}>
            ...
            <nav className="gr-desktop-nav" style={{ display:"none" }}>
              <ul style={{ display:"flex", gap:"36px", listStyle:"none", margin:0, padding:0 }}>
                {navLinks.map((item) => (
                  <li key={item.name}><a href={item.link} className="gr-link" style={{ fontSize:"12px", fontWeight:600, color:"#27272a", letterSpacing:"0.08em", textTransform:"uppercase" }}>{item.name}</a></li>
                ))}
              </ul>
            </nav>
            <div style={{ display:"flex", alignItems:"center", gap:"2px" }}>
              <div className="gr-desktop-search" style={{ display:"flex", alignItems:"center" }}>
                <div className={`gr-search-wrap ${searchOpen ? "open" : ""}`}>
                  <input ref={searchRef} type="text" placeholder="Search products..." onBlur={closeSearch} style={{ width:"100%", border:"none", borderBottom:"1.5px solid #f97316", background:"transparent", padding:"4px 8px", fontSize:"13px", outline:"none", color:"#18181b", fontFamily:"'DM Sans',sans-serif" }}/>
                </div>
                <button className="gr-icon-btn" onClick={toggleSearch}><SearchIcon /></button>
              </div>
              <button className="gr-icon-btn" onClick={() => dispatch(toggleWishlist("__nav__"))} style={{ color: wishlistActive ? "#f97316" : "#27272a" }}>
                <HeartIcon filled={wishlistActive} />
              </button>
              <button className="gr-icon-btn" onClick={() => dispatch(setShowCart(true))} style={{ position:"relative" }}>
                <BagIcon />
                {cartCount > 0 && (
                  <span style={{ position:"absolute", top:"6px", right:"5px", background:"#f97316", color:"#fff", fontSize:"9px", fontWeight:700, width:"16px", height:"16px", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", border:"2px solid #fff" }}>{cartCount}</span>
                )}
              </button>
      ...
      <aside className={`gr-nav-drawer gr-root ${showMenu ? "open" : ""}`}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"22px 28px 18px", borderBottom:"1px solid #f4f4f5" }}>
          <span className="gr-logo" style={{ fontSize:"24px", color:"#18181b" }}>Gr<span style={{ color:"#f97316" }}>O</span>cify<sup style={{ fontSize:"9px", fontFamily:"'DM Sans',sans-serif", fontWeight:600, letterSpacing:"0.12em", color:"#a3a3a3", marginLeft:"3px" }}>FRESH</sup></span>
          <button onClick={closeMenu} style={{ background:"none", border:"none", cursor:"pointer", color:"#71717a", padding:"6px", borderRadius:"8px", lineHeight:0, transition:"color .2s" }} onMouseOver={e=>e.currentTarget.style.color="#f97316"} onMouseOut={e=>e.currentTarget.style.color="#71717a"}><CloseIcon /></button>
        </div>
        ...
        <div style={{ padding:"24px 28px 0", flex:1, overflowY:"auto" }}>
          ...
          <ul style={{ listStyle:"none", margin:0, padding:0 }}>
            {navLinks.map((item) => (
              <li key={item.name}><div className="gr-drawer-link" onClick={() => { navigate(item.link); closeMenu(); }}>{item.name}<span className="gr-arrow"><ArrowIcon /></span></div></li>
            ))}
          </ul>
        </div>
        <div style={{ padding:"20px 28px", display:"flex", gap:"10px" }}>
          <button style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", padding:"12px", borderRadius:"12px", background:"#fff7ed", border:"1.5px solid #fed7aa", color:"#ea580c", fontSize:"12px", fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}><HeartIcon /> Wishlist</button>
          <button onClick={() => { closeMenu(); dispatch(setShowCart(true)); }} style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", padding:"12px", borderRadius:"12px", background:"#f97316", border:"none", color:"#fff", fontSize:"12px", fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
            <BagIcon /> Cart {cartCount > 0 && <span style={{ background:"#fff", color:"#f97316", borderRadius:"999px", fontSize:"10px", fontWeight:700, padding:"1px 7px" }}>{cartCount}</span>}
          </button>
        </div>
        <div style={{ padding:"14px 28px 28px", borderTop:"1px solid #f4f4f5" }}>
          <div style={{ display:"flex", gap:"20px", marginBottom:"10px" }}>
            {["FB","IG","TW","LI"].map(s => (<a key={s} href="#" style={{ fontSize:"11px", fontWeight:700, color:"#71717a", textDecoration:"none" }} onMouseOver={e=>e.currentTarget.style.color="#f97316"} onMouseOut={e=>e.currentTarget.style.color="#71717a"}>{s}</a>))}
          </div>
      ...
      <aside className={`gr-cart-drawer ${showCart ? "open" : ""}`}>
        ...
        {cartItems.length > 0 && (
          <div style={{ padding:"20px 24px 28px", borderTop:"1px solid #f4f4f5" }}>
            ...
            <button style={{ width:"100%", background:"#f97316", color:"#fff", border:"none", borderRadius:14, padding:"15px", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", marginBottom:10 }} onMouseOver={e=>e.currentTarget.style.background="#ea580c"} onMouseOut={e=>e.currentTarget.style.background="#f97316"}>
              Checkout → {fmt(cartTotal)}
            </button>
            <button onClick={() => dispatch(clearCart())} style={{ width:"100%", background:"transparent", color:"#a3a3a3", border:"1.5px solid #e5e5e5", borderRadius:14, padding:"12px", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }} onMouseOver={e=>{ e.currentTarget.style.borderColor="#ef4444"; e.currentTarget.style.color="#ef4444"; }} onMouseOut={e=>{ e.currentTarget.style.borderColor="#e5e5e5"; e.currentTarget.style.color="#a3a3a3"; }}>
              Clear Cart
            </button>
          </div>
        )}
